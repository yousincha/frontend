...
const FileUpload2 = ({ images, onImageChange }) => {
  ...
  const handleDrop = async (files) => {
    ...
    try {
      const response = await axiosInstance.post("/image", formData, config);
      const newImages = [...images, response.data.fileName];
      onImageChange(newImages);
    } catch (error) {
      console.error(error);
    }
...
// PropTypes declaration for the component
