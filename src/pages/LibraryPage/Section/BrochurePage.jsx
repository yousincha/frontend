...
const BrochurePage = () => {
  ...
  const fetchBrochureFiles = async () => {
    try {
      const response = await axiosInstance.get("/library/brochure");
      setBrochureFiles(response.data);
    } catch (error) {
  ...
  const handleDownload = (fileName) => {
    ...
    link.href = `${apiUrl}/library/brochure/${encodeURIComponent(fileName)}`;
  ...
  const renderButtonContent = () => {
    const contentArray = brochureFiles.map((filename) => [
      "brochure",
      filename,
    ]);
    ...
    <div>
      <div className="library-body " style={{ display: "flex", gap: 10 }}>
        <div className="page-right" style={{ width: "100%", height: "85vh" }}>
          ...
          >
            LIBRARY &gt; <span style={{ color: "navy" }}>브로셔</span>
          </div>
          <div
            style={{
              display: "flex",
